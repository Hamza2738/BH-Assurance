...
import { AuthService } from '../../application-gestion-conge/services/auth.service'; // <-- ajuste le chemin si besoin
...
export class NavService {
  ...
  // ====== Rôles autorisés pour le menu Admin ======
  private readonly ADMIN_ROLES = [1, 2];

  // ====== Menus par rôle ======
  // Admin / Super Admin
  private readonly ADMIN_MENU: Menu[] = [
    { title: 'Home', icon: 'home', type: 'link', path: '/application-gestion-conge/pages/home/home' },
    { title: 'Gestion Demandes', icon: 'file-text', type: 'link', path: '/application-gestion-conge/pages/gestion-demande/gestion-demande' },
    {
      title: 'Gestion Utilisateurs', icon: 'users', type: 'sub', active: false, children: [
        { path: '/application-gestion-conge/pages/ajout-utilisateur/ajout-utilisateur', title: 'Ajouter utilisateur', type: 'link' },
        { path: '/application-gestion-conge/pages/gestion-utilisateur/gestion-utilisateur', title: 'Gestion Utilisateurs', type: 'link' },
      ]
    },
    { title: 'Gestion Rôles', icon: 'shield', type: 'link', path: '/application-gestion-conge/pages/gestion-role/gestion-role' },
    { title: 'Gestion Grades', icon: 'award', type: 'link', path: '/application-gestion-conge/pages/gestion-grade/gestion-grade' },
    { title: 'Gestion Départements', icon: 'layers', type: 'link', path: '/application-gestion-conge/pages/gestion-departement/gestion-departement' },
    { title: 'Gestion Types Congé', icon: 'tag', type: 'link', path: '/application-gestion-conge/pages/gestion-type-conge/gestion-type-conge' },
  ];

  // Employé & autres rôles
  private readonly EMPLOYEE_MENU: Menu[] = [
	{ title: 'Home', icon: 'home', type: 'link', path: '/application-gestion-conge/pages/home/home' },
    { title: 'Gestion Demandes', icon: 'file-text', type: 'link', path: '/application-gestion-conge/pages/gestion-demande/gestion-demande' },
    { title: 'Ajout Demande', icon: 'plus-square', type: 'link', path: '/application-gestion-conge/pages/ajout-demande/ajout-demande' },
    { title: 'Consulter solde de congés', icon: 'calendar', type: 'link', path: '/application-gestion-conge/pages/gestion-user-conge/gestion-user-conge' },
    { title: 'Historiques', icon: 'clock', type: 'link', path: '/application-gestion-conge/pages/gestion-historique/gestion-historique' },
  ];

  // Stream consommé par le composant de la navbar
  ...
  constructor(private auth: AuthService) {
    ...
    // Init selon l'utilisateur déjà stocké
    const initialRole = this.auth.currentUserValue?.role_id ?? null;
    this.items.next(this.menuForRole(initialRole));

    // Mise à jour live (login / logout / changement de rôle)
    this.auth.getUserObservable().subscribe(user => {
      const roleId = user?.role_id ?? null;
      this.items.next(this.menuForRole(roleId));
    });
  }
  ...
  // Sélectionne le bon menu selon le rôle
  private menuForRole(roleId: number | null): Menu[] {
    if (roleId != null && this.ADMIN_ROLES.includes(roleId)) {
      return this.ADMIN_MENU;
    }
    if (roleId == null) {
      // Non connecté : menu vide (ou tu peux mettre uniquement 'Home' si tu veux)
      return [];
    }
    return this.EMPLOYEE_MENU;
  }

  /* ===========================================================
   *  TOUS LES AUTRES LIENS DU TEMPLATE (INUTILES ICI) — COMMENTÉS
   *  => Tu peux les réactiver plus tard si besoin.
   * ===========================================================

  private readonly TEMPLATE_EXTRA_MENU: Menu[] = [
    {
